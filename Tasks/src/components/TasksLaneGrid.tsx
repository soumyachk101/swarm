...
import TasksStatusLane from './TasksStatusLane.js';
...
export default function TasksLaneGrid({
  ...
  return (
    <div className="flex gap-3 h-full min-h-0 px-4 py-3" style={{ minWidth: `${columns.length * (columnWidth + 12)}px` }}>
      {columns.map((col) => (
        <TasksStatusLane key={col.id} column={col} tasks={tasksByColumn.get(col.id) ?? []}
