  ...
  return (
    <div className="h-full overflow-y-auto scrollbar-sleek px-4 py-3">
      ...
      <div className="flex items-baseline justify-between">
        <span className="text-mini font-semibold text-bee-text">Mission progress</span>
        <span className="text-mini text-bee-textMuted">
      ...
      <div className="mt-1 text-micro text-bee-textMuted">{pct}% complete</div>
      ...
      <div className="mt-4 grid grid-cols-5 gap-1.5">
        {COLUMN_META.map((c) => {
          ...
          return (
            <div
              key={c.id}
              className="rounded-md border border-bee-border/40 glass-inset px-2 py-1.5"
            >
              <div className="flex items-center gap-1">
                <span className="size-1.5 shrink-0 rounded-full" style={{ background: c.color }} />
                <span className="truncate text-micro uppercase tracking-wide text-bee-textMuted">
                  {c.label}
                </span>
              </div>
              <div className="mt-0.5 text-sm font-semibold text-bee-text">{n}</div>
            </div>
          );
        })}
      </div>
      ...
      {/* ── Stage rundown ──────────────────────────────────── */}
      <div className="mt-4 text-micro font-semibold uppercase tracking-wider text-bee-gold">
        Stages
      </div>
        ...
        {stages.map((s) => {
          ...
          return (
            ...
            >
              ...
              />
              <span className="w-24 shrink-0 truncate text-micro font-medium text-bee-text">
                {s.title}
              </span>
              <span className="flex-1 truncate text-micro text-bee-textMuted">{s.statusText}</span>
              <span
