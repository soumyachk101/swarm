...
export default function TaskCombStatusLane({
  ...
  return (
    <section data-workhive-status-drop-target data-workhive-status={column.id}
    ...
    >
      ...
      <div className="flex items-center justify-between px-3 py-2 border-b border-bee-border/50 shrink-0">
        <div className="flex items-center gap-2 min-w-0">
          <span className="text-micro font-semibold text-bee-text uppercase tracking-wider truncate">{column.title}</span>
          <span className="text-micro font-mono text-bee-textMuted bg-bee-border/30 px-1.5 py-0.5 rounded-full shrink-0">{tasks.length}</span>
        </div>
        {onAddTask && (
      ...
      <div className="flex-1 overflow-y-auto p-2 space-y-2 min-h-0">
        {tasks.length === 0 ? (
          <div className="text-micro text-bee-textMuted text-center py-4 italic">No tasks</div>
        ) : (
          tasks.map((task) => (
        ...
        {onAddTask && (
          <button onClick={() => onAddTask(column.id)}
            className="w-full flex items-center justify-center gap-1 py-1.5 rounded-lg text-micro text-bee-textMuted hover:text-bee-goldHi hover:bg-bee-border/30 transition-colors opacity-0 hover:opacity-100">
