...
export function useTaskCombCardPointerDrag(
  ...
  const createPreview = useCallback((cardEl: HTMLElement) => {
    ...
    clone.setAttribute('data-workhive-board-card-drag-preview', 'true');
  ...
  const showDropIndicator = useCallback((x: number, y: number) => {
    ...
    if (!el) return;
    const card = el.closest<HTMLElement>('[data-workhive-board-card-id]');
    if (!card) return;
    const rect = card.getBoundingClientRect();
    const indicator = document.createElement('div');
    indicator.setAttribute('data-workhive-board-card-drop-indicator', 'true');
    indicator.style.position = 'fixed';
    indicator.style.zIndex = '2147483646';
    indicator.style.height = '3px';
    indicator.style.borderRadius = '9999px';
    indicator.style.background = 'var(--bee-gold-hex, #c9a227)';
    indicator.style.boxShadow = '0 0 0 1px rgb(var(--bee-gold) / 0.24), 0 8px 20px rgb(var(--bee-gold) / 0.36)';
  ...
  const stopDrag = useCallback(
    (commit: boolean) => {
      ...
      if (commit && drag.hasStarted) {
        ...
        if (el) {
          const lane = el.closest<HTMLElement>('[data-workhive-status-drop-target]');
          if (lane) {
            targetColumn = lane.getAttribute('data-workhive-status') as ColumnId || drag.sourceColumn;
          }
          const targetCard = el.closest<HTMLElement>('[data-workhive-board-card-id]');
          if (targetCard) {
            const rect = targetCard.getBoundingClientRect();
            const allCards = Array.from(document.querySelectorAll<HTMLElement>('[data-workhive-board-card-id]'));
      ...
      document.querySelectorAll('[data-workhive-board-card-pointer-dragging]').forEach((el) => {
        (el as HTMLElement).style.outline = '';
        el.removeAttribute('data-workhive-board-card-pointer-dragging');
      });
  ...
  const onPointerDownCapture = useCallback(
    (e: React.PointerEvent) => {
      ...
      const cardEl = (e.target as HTMLElement).closest<HTMLElement>('[data-workhive-board-card-id]');
      if (!cardEl) return;

      const taskId = cardEl.getAttribute('data-workhive-board-card-id');
      if (!taskId) return;

      const tag = (e.target as HTMLElement).tagName;
      if (tag === 'BUTTON' || tag === 'INPUT' || tag === 'SELECT' || tag === 'TEXTAREA') return;

      const lane = cardEl.closest<HTMLElement>('[data-workhive-status]');
      const sourceColumn = (lane?.getAttribute('data-workhive-status') ?? 'backlog') as ColumnId;

      const isSelected = cardEl.getAttribute('data-workhive-board-card-selected') === 'true';
      let allIds: string[];
      if (isSelected) {
        allIds = Array.from(document.querySelectorAll<HTMLElement>('[data-workhive-board-card-selected="true"]'))
          .map((el) => el.getAttribute('data-workhive-board-card-id')!);
      } else {
        allIds = [taskId];
      }
      ...
      const onPointerMove = (ev: PointerEvent) => {
        ...
        if (!dragRef.current.hasStarted && (dx >= DRAG_THRESHOLD_PX || dy >= DRAG_THRESHOLD_PX)) {
          ...
          cardEl.style.outline = '1px solid var(--bee-gold-hex, #c9a227)';
          cardEl.setAttribute('data-workhive-board-card-pointer-dragging', 'true');
