...
function TaskRow({ t, statuses }: { t: TaskCard; statuses: Record<string, string> }) {
  ...
  return (
    <div className="rounded-md glass-inset px-2.5 py-1.5 transition-colors hover:border-bee-gold/40">
      <div className="flex items-center gap-2 min-w-0">
        <span className="size-1.5 shrink-0 rounded-full" style={{ background: STATUS_COLOR[st] }} />
        <span className="flex-1 truncate text-micro font-medium text-bee-text">{t.title}</span>
        {t.assignedCli && (
          <span className="shrink-0 rounded bg-bee-gold/10 px-1 py-px text-micro font-bold uppercase tracking-wide text-bee-gold">
            {t.assignedCli}
          </span>
        )}
        {t.assignedRole && (
          <span className="shrink-0 text-micro uppercase tracking-wide text-bee-textMuted">
            {t.assignedRole}
          </span>
        )}
      </div>
      ...
      {t.description && (
        <div className="mt-0.5 truncate pl-3.5 text-micro leading-[1.35] text-bee-textMuted">
          {t.description}
        </div>
      )}

      <div className="mt-1 flex flex-wrap items-center gap-2.5 pl-3.5 text-micro text-bee-textMuted">
        {t.worktreeBranch && (
...
export default function TaskListBoard({ tasks, statuses = {} }: TaskListBoardProps) {
  if (tasks.length === 0) {
    return (
      <div className="flex h-full items-center justify-center text-mini text-bee-textMuted">
        ...
        return (
          <div key={c.id}>
            <div className="flex items-center gap-1.5">
              ...
              <span className="text-micro font-semibold uppercase tracking-wider text-bee-textDim">
                {c.label}
              </span>
              <span className="text-micro text-bee-textMuted">{rows.length}</span>
            </div>
