...
describe("mergeMcpJson", () => {
  ...
  it("keeps servers it does not manage", () => {
    // Nectar writes itself into this file; the toolbox must not evict it.
    const existing = JSON.stringify({ mcpServers: { nectar: { command: "node", args: ["server.js"] } } });
    const out = JSON.parse(mergeMcpJson(existing, { ...EMPTY_TOOLBOX, mcpServers: [server("playwright")] }));
    expect(Object.keys(out.mcpServers).sort()).toEqual(["nectar", "playwright"]);
  });
...
describe("toolboxTargets", () => {
  ...
  it("is empty for an unbound workhive", () => {
