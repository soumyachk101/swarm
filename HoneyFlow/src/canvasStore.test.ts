...
describe("canvas arrangement", () => {
  ...
  it("does not disturb another workhive's layout", () => {
    // The canvas only ever sees the ACTIVE hive's panes. Laying those out must
    // not touch geometry belonging to panes that are simply not on screen.
    const s = useCanvasStore.getState();
    s.ensureNode("hiveA-1", ["hiveA-1"]);
    useCanvasStore.getState().moveNode("hiveA-1", 900, 700);
    useCanvasStore.getState().ensureNode("hiveB-1", ["hiveB-1"]);
    const n = useCanvasStore.getState().nodes["hiveA-1"];
  ...
  it("keeps a camera per workhive", () => {
    const s = useCanvasStore.getState();
    s.setCamera("hiveA", { x: 10, y: 20, zoom: 1.5 });
    useCanvasStore.getState().setCamera("hiveB", { x: -5, y: 0, zoom: 0.8 });
    expect(useCanvasStore.getState().cameraFor("hiveA")).toEqual({ x: 10, y: 20, zoom: 1.5 });
    expect(useCanvasStore.getState().cameraFor("hiveB")).toEqual({ x: -5, y: 0, zoom: 0.8 });
  });

  it("defaults an unseen workhive to the origin at 100%", () => {
