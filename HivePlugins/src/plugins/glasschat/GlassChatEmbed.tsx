...
const DEFAULT_TEAM_ID = ENV.VITE_GLASSCHAT_TEAM_ID ?? "team_hiveory_workhive";
...
export function GlassChatEmbed({
  ...
  if (isConfiguring) {
    return (
      <div className="flex h-full flex-col items-center justify-center p-5 text-center text-bee-text overflow-y-auto scrollbar-sleek">
        ...
        <p className="mt-1 max-w-[36ch] text-mini text-bee-textMuted leading-relaxed">
        ...
        <div className="mt-4 w-full max-w-xs space-y-3 text-left">
          <div>
            <label className="block text-mini font-medium text-bee-gold mb-1">
              App ID
            </label>
            <input
              type="text"
              ...
              placeholder="app_..."
              className="w-full rounded-md border border-bee-border/60 glass-inset px-2.5 py-1.5 text-xs text-bee-text font-mono outline-none focus:border-bee-gold/60"
            />
          </div>

          <div>
            <label className="block text-mini font-medium text-bee-textDim mb-1">
          ...
          {error && <p className="text-micro text-bee-err mt-1 text-center">{error}</p>}
  ...
  return (
    <div className="relative flex h-full w-full flex-col overflow-hidden glass-body">
      {/* Top Banner Header */}
      <div className="flex items-center justify-between border-b border-bee-border/40 glass-hi px-3 py-1.5 text-mini">
        ...
        <div className="flex items-center gap-2">
          <button
            ...
            className="flex items-center gap-1 text-bee-textMuted hover:text-bee-gold text-micro p-1 rounded hover:bg-bee-border/30 transition-colors"
          ...
          <button
            ...
            className="flex items-center gap-1 text-bee-textMuted hover:text-bee-text text-micro"
      ...
      <div className="relative flex-1 overflow-hidden">
        {loading ? (
          <div className="absolute inset-0 flex flex-col items-center justify-center glass-hi backdrop-blur-sm z-10 text-bee-textMuted">
            ...
            <span className="text-micro text-bee-textMuted mt-0.5">Provisioning Partner Session</span>
          </div>
        ) : error ? (
          <div className="flex h-full flex-col items-center justify-center p-4 text-center">
            <p className="text-xs text-bee-err font-medium mb-1">Partner Session Error</p>
            <p className="text-mini text-bee-textMuted max-w-[32ch]">{error}</p>
            <button
              onClick={initPartnerSession}
