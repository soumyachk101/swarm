...
import { beeVoiceHost } from "./host.js";
...
export default function VoiceHotkeys() {
  ...
  return (
    <div className="fixed bottom-8 left-1/2 z-[200] -translate-x-1/2 flex items-center gap-2 rounded-full border border-bee-gold/40 glass-hi px-3.5 py-1.5 text-mini font-medium text-bee-goldHi shadow-xl backdrop-blur">
...
/** Type into the QueenBee's CLI, revealing it first so the user sees it land.
 *  No trailing Enter — the transcript waits at the prompt for review. */
function typeIntoQueenBee(text: string) {
  beeVoiceHost().revealQueen();
  beeVoiceHost().deliverToQueen(text);
}
