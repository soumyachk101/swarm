...
export class PheromoneDatabase {
  ...
  constructor(projectPath: string, db: initSqlJs.Database) {
    ...
    this.db = db;
    this.dbPath = path.join(projectPath, '.pheromone', 'pheromone.db');
    
    this.migrate();
  }

  static async create(projectPath: string): Promise<PheromoneDatabase> {
    const SQL = await initSqlJs();
    const dbPath = path.join(projectPath, '.pheromone', 'pheromone.db');
    ...
    return new PheromoneDatabase(projectPath, db);
  }
