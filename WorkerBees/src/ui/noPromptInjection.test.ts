...
// Hiveory must never type text into an agent's prompt. A pasted charter or
...
const PANE = readFileSync(join(__dirname, "WorkerBeePane.tsx"), "utf8");

describe("no prompt injection", () => {
  it("never sends a QueenBee charter into a CLI", () => {
    expect(PANE).not.toMatch(/MODE_SYSTEM_PROMPTS/);
    expect(PANE).not.toMatch(/queenCharter/);
  });
  ...
  it("guards that pointer behind a bail-out for every MCP-capable CLI", () => {
    const guard = PANE.indexOf("MCP_CAPABLE_CLIS.includes(workerBee.cli)");
