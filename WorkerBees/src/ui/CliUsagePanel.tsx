...
/** CLIs whose transcripts Hiveory knows how to read. */
const TRACKED = ["claude", "codex", "opencode"];

const BUDGET_KEY = "hive_cli_budgets";
...
// hive palette so the panel reads as one surface; the mark on it is the
// vendor's, so a row is identifiable before you read the name.
const TILE: Record<string, { bg: string }> = {
  claude: { bg: "bg-bee-gold/12" },
  codex: { bg: "bg-bee-border/50" },
  opencode: { bg: "bg-bee-amber/12" },
};
...
function Meter({
  ...
  const tone =
    left === null ? "text-bee-text"
    : left < 15 ? "text-bee-err"
    : left < 40 ? "text-bee-warn"
    : "text-bee-gold";
  const fill =
    left === null ? "bg-bee-border"
    : left < 15 ? "bg-bee-err"
    : left < 40 ? "bg-bee-warn"
    : "bg-bee-gold";
  return (
    <div className="space-y-1.5">
      <div className="flex items-baseline gap-1.5">
        <span className="text-xs text-bee-textDim">{label}</span>
        <span className={`ml-auto text-xs font-bold tabular-nums ${tone}`}>
          {left !== null ? `${left}% left` : `${compact(used)} ${tokens ? "tokens" : "msgs"}`}
        </span>
        {note ? (
          <span className="text-mini text-bee-textMuted">· {note}</span>
        ) : resets ? (
          <span className="text-mini text-bee-textMuted">· resets in {resets}</span>
        ) : null}
      </div>
      ...
      {pct !== null && (
        <div className="h-2 overflow-hidden rounded-full bg-bee-border/40">
          <div
      ...
      {window && tokens && (
        <div className="flex flex-wrap items-center gap-x-2.5 gap-y-0.5 text-micro tabular-nums text-bee-textMuted">
          ...
          <span className="text-bee-textMuted/70">
  ...
  return (
    <div className="flex w-[360px] flex-col overflow-hidden rounded-2xl glass-hi glass-sheen shadow-2xl shadow-black/70">
      <div className="flex items-center gap-2 px-4 pb-2 pt-3.5">
        <span className="text-xs font-bold uppercase tracking-[0.12em] text-bee-text">
          Plan limits
        </span>
        <button
          onClick={refresh}
          className="ml-auto rounded p-1 text-bee-textMuted transition-colors hover:bg-bee-border/40 hover:text-bee-text"
          title="Rescan transcripts"
        >
          <RefreshCw className={`size-3.5 ${loading ? "animate-spin text-bee-gold" : ""}`} />
        </button>
        {onClose && (
          <button
            onClick={onClose}
      ...
      <div className="max-h-[62vh] divide-y divide-bee-border/30 overflow-y-auto scrollbar-sleek">
        {error && <p className="px-4 py-3 text-mini text-bee-err">{error}</p>}
        {!error && rows.length === 0 && !loading && (
          <p className="px-4 py-3 text-mini text-bee-textMuted">
          ...
          return (
            <div key={r.cli} className="space-y-3 px-4 py-3.5">
              <div className="flex items-center gap-2.5">
                <div className={`flex size-8 shrink-0 items-center justify-center rounded-lg text-bee-textDim ${tile.bg}`}>
                  {brand ? <BrandGlyph brand={brand} size={17} /> : <WorkerBeeMark size={17} />}
                </div>
                <span className="text-lg font-bold text-bee-text">{name}</span>
                {r.plan && (
                  <span className="rounded-full border border-bee-gold/25 bg-bee-gold/10 px-1.5 py-px text-micro font-semibold uppercase tracking-wide text-bee-goldHi">
                    {r.plan}
                  </span>
                )}
                <button
                  onClick={() => setEditing(editing === r.cli ? null : r.cli)}
                  className="ml-auto text-micro text-bee-textMuted transition-colors hover:text-bee-gold"
              ...
              {!r.installed ? (
                <p className="text-xs text-bee-textMuted">Not installed.</p>
              ) : idle ? (
                <p className="text-xs text-bee-textMuted">No recent {name} sessions</p>
              ) : (
              ...
              {editing === r.cli && (
                <div className="space-y-1.5 rounded-lg border border-bee-border/50 glass-inset p-2.5">
                  <p className="text-micro leading-relaxed text-bee-textMuted">
                  ...
                  {(["fiveHour", "weekly"] as const).map((field) => (
                    <label key={field} className="flex items-center gap-2 text-micro text-bee-textDim">
                      ...
                      <input
                        ...
                        className="min-w-0 flex-1 rounded border border-bee-border/60 glass-inset px-1.5 py-0.5 text-micro text-bee-text outline-none focus:border-bee-gold/50"
                      />
                    </label>
                  ))}
                </div>
      ...
      <div className="flex flex-col items-center gap-1 border-t border-bee-border/30 px-4 py-3">
        {detailsFor && (
          <button
            onClick={() => shellOpen(USAGE_PAGE[detailsFor.cli]).catch(() => {})}
            className="flex items-center gap-1.5 text-xs font-medium text-bee-textDim transition-colors hover:text-bee-gold"
          >
            View usage details
            <ArrowRight className="size-3.5" />
          </button>
        )}
        <span className="text-micro text-bee-textMuted/70">
