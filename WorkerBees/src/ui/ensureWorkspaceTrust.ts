...
export async function ensureClaudeWorkspaceTrust(cwd: string): Promise<void> {
  ...
  console.log(`[Nectar] Claude workhive trust pre-accepted for: ${cwd}`);
}
...
/**
 * Best-effort trust / auto-approve setup for every WorkerBee CLI before spawn.
 * Claude is the one with the hard workhive-trust dialog; others get a no-op
...
export async function ensureCliWorkspaceTrust(cli: string, cwd: string): Promise<void> {
  ...
  try {
    ...
    console.warn(`[Nectar] WorkHive trust setup failed for ${cli}:`, e);
  }
}
