...
// `@hiveory/nectar` PlanManager (Nectar/src/plans) — this file only maps MCP
// tool calls onto it and, on completion, harvests the summary into searchable
// memory so future sessions inherit the lesson.
import { PlanManager, MemoryManager, NectarDatabase } from '@hiveory/nectar';

type ToolResult = { text: string };

export const PLAN_TOOLS = [
  {
    name: 'nectar_create_plan',
    description:
...
export async function runPlanTool(
  ...
  switch (name) {
    case 'nectar_create_plan': {
      ...
      return {
        text:
          ...
          ` Use nectar_update_plan_step with plan_id "${plan.id}" as you work.`,
      };
    }
    case 'nectar_update_plan_step': {
    ...
    case 'nectar_list_plans': {
      const plans = await pm.listPlans();
      if (!plans.length) return { text: 'No plans yet. Create one with nectar_create_plan.' };
      const text = plans
        .map((p) => {
...
/**
 * Knowledge harvest: append the plan's summary to `.nectar/memory/decisions.md`
 * and index it, so `nectar_query` surfaces it in future sessions.
 */
