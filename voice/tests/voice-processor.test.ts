...
describe('Voice', () => {
  let Voice: typeof import('../src/voice-processor.js').Voice;
  ...
  it('can be constructed with injected engine', () => {
    const bv = new Voice({ engine: makeFakeEngine() });
    expect(bv).toBeDefined();
  });

  it('transcribeFile delegates to engine', async () => {
    const engine = makeFakeEngine();
    const bv = new Voice({ engine });
  ...
  it('emits error event when dictation fails', async () => {
    ...
    const bv = new Voice({ engine: failingEngine });
    const error = vi.fn();
