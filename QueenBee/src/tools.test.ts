...
function ctx(over: Partial<ToolContext> = {}): ToolContext {
  return {
    createWorkHive: vi.fn(() => "ws-1"),
    listWorkHives: vi.fn(() => [{ id: "ws-1", name: "Default" }]),
    addTask: vi.fn(),
    ...
    deleteWorkHive: vi.fn(() => true),
    renameWorkHive: vi.fn(() => true),
    recolorWorkHive: vi.fn(() => true),
    switchWorkHive: vi.fn(() => true),
...
describe("executeTool", () => {
  it("creates a workhive", () => {
    const c = ctx();
    expect(executeTool("Steward", "create_workhive", { name: "New" }, c)).toContain("New");
    expect(c.createWorkHive).toHaveBeenCalledWith("New");
  });
  ...
  it("performs workhive + worker-bee management", () => {
    const c = ctx();
    expect(executeTool("Steward", "rename_workhive", { id: "ws-1", name: "X" }, c)).toContain("Renamed");
    expect(executeTool("Steward", "switch_workhive", { id: "ws-1" }, c)).toContain("Switched");
  ...
  it("surfaces missing entities", () => {
    expect(() => executeTool("Steward", "delete_workhive", { id: "zzz" }, ctx({ deleteWorkHive: () => false }))).toThrow(/No workhive/);
  ...
  it("management tools are Steward-only", () => {
    for (const name of ["delete_workhive", "set_grid_layout", "set_left_sidebar", "remove_worker_bee"]) {
...
describe("a queen sees its own project's worktrees", () => {
...
describe("summoning bees with a model and effort", () => {
  it("passes the model and effort through to the host", () => {
    const launch = vi.fn();
    executeTool("Steward", "launch_worker_bee", { cli: "claude", model: "opus", effort: "medium" }, ctx({ launchWorkerBee: launch }));
    expect(launch).toHaveBeenCalledWith("claude", undefined, { model: "opus", effort: "medium" });
  });

  it("reports back what it summoned", () => {
    const out = executeTool("Steward", "launch_worker_bee", { cli: "claude", name: "Builder", model: "opus", effort: "medium" }, ctx());
  ...
  it("still works with no model or effort at all", () => {
    const launch = vi.fn();
    const out = executeTool("Steward", "launch_worker_bee", { cli: "codex" }, ctx({ launchWorkerBee: launch }));
  ...
  it("rejects an effort level outside the allowed set", () => {
    expect(() =>
      executeTool("Steward", "launch_worker_bee", { cli: "claude", effort: "ludicrous" }, ctx()),
    ).toThrow(ToolError);
  });

  it("stays closed to read-only modes", () => {
    expect(() => executeTool("Forager", "launch_worker_bee", { cli: "claude" }, ctx())).toThrow(ToolError);
  });
});
