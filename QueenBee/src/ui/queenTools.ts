...
// QueenBee's powers, unchanged from when she was a chat agent — only the caller
// moved. The crowned WorkerBee's CLI reaches these through the queenbee MCP
// server and the request bridge in ./queenBridge.
//
// Everything is scoped to the queen's OWN folder: with several folders open,
// each queen sees only its own board, panes and trees. Synchronous tools run
// against the workhive bindings the app registered (ToolContext); the async
// ones use the host's capabilities plus Nectar directly.

/**
 * Run one QueenBee tool against the live app. `mode` gates it exactly as it did
...
export async function runQueenTool(
  ...
  const wsId = host.workHiveIdForFolder(projectPath);
  if (!wsId) return `Error: no workhive is bound to ${projectPath}.`;
  const ctx = host.toolContext(wsId);
  try {
    // The charter plus a live map of this queen's territory. Answered here (not
...
${host.describeScope(projectPath)}`;
    ...
    if (ASYNC_TOOLS.has(name)) {
      ...
      // Stinger's one action: a read-only security review in its own WorkerBee.
      if (name === 'run_stinger_scan') {
        if (!projectPath) throw new ToolError('No project is open.');
        const scanPath = args.path ? String(args.path) : projectPath;
        host.launchBee(wsId, 'claude', 'Stinger scan', ['--cwd', scanPath]);
        return `Launched a Stinger security review over ${scanPath}. Findings appear in its WorkerBee pane.`;
      }
      ...
      if (name === 'approve_task') {
        ...
        // Through HiveMind so the merge also releases file locks and marks the
      ...
      if (name === 'reject_task') {
        ...
        // The worktree and its locks stay so the bee reworks in place and a
        ...
        return `Rejected ${branch} — handed back to the WorkerBee for rework.${
      ...
      if (name === 'dispatch_goal') {
        ...
        // reported as dispatched, or QueenBee claims work that never ran.
