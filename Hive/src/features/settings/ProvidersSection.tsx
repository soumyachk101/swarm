  ...
  return (
    <div className="space-y-3 glass-inset rounded-lg p-3">
      ...
      <div className="space-y-2">
        {isCustom && (
          <>
            <div className="flex flex-col gap-1">
              <label className="text-micro text-bee-textMuted uppercase tracking-wider">Provider ID</label>
              <input
                type="text"
                value={id}
                onChange={(e) => setId(e.target.value)}
                placeholder="my-provider"
                className="w-full glass-inset border border-bee-border rounded-lg px-2.5 py-1.5 text-xs text-bee-text placeholder-bee-textMuted outline-none focus:ring-1 focus:ring-bee-gold transition-colors font-mono"
              />
            </div>
            <div className="flex flex-col gap-1">
              <label className="text-micro text-bee-textMuted uppercase tracking-wider">Display Name</label>
              <input
                type="text"
                value={name}
                onChange={(e) => setName(e.target.value)}
                placeholder="My Provider"
                className="w-full glass-inset border border-bee-border rounded-lg px-2.5 py-1.5 text-xs text-bee-text placeholder-bee-textMuted outline-none focus:ring-1 focus:ring-bee-gold transition-colors"
              />
            </div>
          </>
        )}
        ...
        <div className="flex flex-col gap-1">
          <label className="text-micro text-bee-textMuted uppercase tracking-wider">API Type</label>
          <select
            value={apiType}
            onChange={(e) => setApiType(e.target.value as ProviderConfig["apiType"])}
            className="w-full glass-inset border border-bee-border rounded-lg px-2.5 py-1.5 text-xs text-bee-text outline-none focus:ring-1 focus:ring-bee-gold transition-colors"
          >
        ...
        {apiType !== "anthropic-messages" && (
          <div className="flex flex-col gap-1">
            <label className="text-micro text-bee-textMuted uppercase tracking-wider">Base URL</label>
            <input
              type="text"
              value={baseUrl}
        ...
        <div className="flex flex-col gap-1">
          <label className="text-micro text-bee-textMuted uppercase tracking-wider">API Key {isCustom && "(optional)"}</label>
          <input
            type="password"
            value={apiKey}
        ...
        {isCustom && (
          <div className="flex flex-col gap-1">
            <label className="text-micro text-bee-textMuted uppercase tracking-wider">Custom Headers</label>
            <div className="space-y-1.5">
              {Object.entries(headers).map(([k, v]) => (
                <div key={k} className="flex items-center gap-1.5">
                  <span className="text-micro font-mono text-bee-textDim flex-1 truncate">
                    {k}: {v}
                  </span>
                  <button
                    onClick={() => {
              ...
              <div className="flex items-center gap-1.5">
                <input
                  ...
                  className="flex-1 glass-inset border border-bee-border rounded px-2 py-1 text-micro text-bee-text placeholder-bee-textMuted outline-none focus:ring-1 focus:ring-bee-gold transition-colors font-mono"
                />
                <input
                  type="text"
                  value={newHeaderVal}
    ...
    <div className="flex items-center gap-3 px-3 py-2.5 hover:bg-bee-border/20 transition-colors rounded-lg">
      ...
      <div className="flex-1 min-w-0">
        <div className="flex items-center gap-1.5">
          ...
          {isCustom && (
            <span className="text-micro px-1 py-0.5 rounded bg-bee-gold/10 text-bee-gold border border-bee-gold/20">
              CUSTOM
            </span>
          )}
        </div>
        {"description" in provider && provider.description ? (
          <p className="text-micro text-bee-textMuted mt-0.5 truncate">{provider.description}</p>
        ) : null}
      </div>
  ...
  return (
    <div className="space-y-4">
      ...
      {connectedProviders.length > 0 && (
        <div>
          <h3 className="text-micro text-bee-textMuted uppercase tracking-wider mb-1.5 font-semibold">
            Connected Providers
          </h3>
          <div className="glass-inset rounded-lg border border-bee-border/30 divide-y divide-bee-border/10">
        ...
        <>
          ...
          <div>
            <button
              ...
              className="flex items-center gap-1 text-micro text-bee-textMuted hover:text-bee-text transition-colors font-semibold uppercase tracking-wider"
            >
            ...
            {showExtra && (
              <div className="mt-1.5 glass-inset rounded-lg border border-bee-border/30 divide-y divide-bee-border/10">
                ...
                {disabledProviders.map((p) => (
                  <div key={p.id} className="flex items-center gap-3 px-3 py-2.5">
                    ...
                    <button
                      ...
                      className="px-2.5 py-1 rounded-lg text-micro bg-bee-gold/10 border border-bee-gold/20 text-bee-goldHi hover:bg-bee-gold/20 transition-colors"
                    >
                      Enable
                    </button>
                  </div>
