...
import { getActiveProjectPath } from "@hiveory/workhive";
  ...
  return (
    <div className="flex h-full flex-col">
      <div className="flex items-center gap-1.5 border-b border-bee-border/30 px-2 py-1.5">
        ...
        {target.diff && (
          <span className="ml-auto shrink-0 rounded bg-bee-gold/10 px-1 py-px text-micro font-bold uppercase text-bee-gold">
            diff
          </span>
        )}
      </div>
      ...
      <div className="flex-1 overflow-auto scrollbar-sleek">
        {loading ? (
          ...
          <div className="px-3 py-2 text-mini text-bee-textMuted">
...
function GitPanel({
  ...
  const Row = ({ e }: { e: GitEntry }) => {
    ...
    const color =
      code === "M" ? "text-bee-gold" : code === "A" ? "text-bee-ok"
      : code === "D" ? "text-bee-err" : "text-bee-textMuted";
    return (
      <div className="group flex items-center gap-1.5 px-2 py-1 text-mini transition-colors hover:bg-bee-border/20">
    ...
    <div className="flex h-full flex-col">
      <div className="shrink-0 border-b border-bee-border/30 px-2.5 py-2">
        <div className="flex items-center gap-2 text-xs">
          ...
          <span className="ml-auto shrink-0 text-micro text-bee-textMuted">
        ...
        <div className="mt-2 flex items-center gap-1">
          <input
            ...
            className="min-w-0 flex-1 rounded-md border border-bee-border/50 glass-inset px-2 py-1 text-mini text-bee-text outline-none transition-colors placeholder:text-bee-textMuted/50 focus:border-bee-gold/40"
          />
          <button
        ...
        {note && <div className="mt-1 truncate text-micro text-bee-textMuted">{note}</div>}
      </div>
      ...
      <div className="flex-1 overflow-y-auto scrollbar-sleek py-1">
        {entries.length === 0 ? (
          ...
          <>
            {staged.length > 0 && (
              <>
                <div className="px-2 py-1 text-micro font-semibold uppercase tracking-wider text-bee-gold">
            ...
            {unstaged.length > 0 && (
              <>
                <div className="px-2 pb-1 pt-2 text-micro font-semibold uppercase tracking-wider text-bee-textDim">
...
export default function ADERightDock({
  ...
  // A fresh promotion should show the new QueenBee, not whatever tab was open.
  const queenId = useWorkerBeesStore((s) => s.workerBees.find((b) => b.isQueen)?.id ?? null);
  useEffect(() => {
    if (queenId) setActiveTab("chat");
  }, [queenId]);
  ...
  return (
    <div
      ...
      className={
        isExpanded
          ...
          : "relative h-full flex flex-col glass-rail border-l border-bee-border/50"
      }
    ...
    >
      ...
      <div className="flex-1 min-h-0 overflow-hidden">
        {/* Hidden rather than unmounted: the QueenBee tab hosts a live CLI, and
