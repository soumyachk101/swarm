...
describe("plane migration (honeyflow renamed to honeyboard)", () => {
  it("renames the old board plane", () => {
    expect(migratePlaneState({ active: "honeyflow", view: "board" }).active).toBe("honeyboard");
  });
  ...
  it("falls back to the board for anything unrecognised", () => {
    expect(migratePlaneState({ active: "wat" }).active).toBe("honeyboard");
    expect(migratePlaneState(undefined).active).toBe("honeyboard");
  });
