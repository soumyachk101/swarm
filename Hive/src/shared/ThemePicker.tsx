      ...
      <button
        ...
        className={`rounded-md p-1.5 transition-colors ${
          open
            ...
            : "text-bee-textMuted hover:bg-bee-border/60 hover:text-bee-text"
        }`}
      ...
      {open &&
        ...
        createPortal(
          <>
            ...
            >
              <div className="px-2 pb-1.5 pt-1 text-micro font-semibold uppercase tracking-wider text-bee-textMuted">
                Theme
              </div>
              {THEMES.map((t) => (
                <ThemeRow
  ...
  return (
    <button
      ...
      className={`flex w-full items-center gap-3 rounded-lg border px-2 py-2 text-left transition-colors ${
        active
          ? "border-bee-gold/40 bg-bee-gold/[0.08] text-bee-text"
    ...
    >
      ...
      <span className="min-w-0 flex-1">
        ...
        <span className="mt-0.5 block text-micro leading-snug text-bee-textMuted">
          {theme.description}
        </span>
      </span>
      {active && <Check className="size-4 shrink-0 text-bee-goldHi" />}
    </button>
  );
}
