...
///
/// Lives in `@hiveory/nectar-mcp` at `Nectar/nectar-mcp/dist/server.js`.
/// Must NOT depend solely on process cwd — `nectar_ensure_structure` used to
/// `set_current_dir` into the opened project, which made cwd-relative probes
/// miss the monorepo path and silently disabled Nectar MCP for WorkerBees.
#[tauri::command]
async fn get_nectar_mcp_path() -> Result<String, String> {
    let mut candidates: Vec<std::path::PathBuf> = Vec::new();

    // Stable anchors relative to this crate (Hive/src-tauri), independent of cwd.
    ...
    // Optional cwd-relative fallbacks (dev from Hive/ or repo root).
    ...
    Err(format!(
        "Nectar MCP server not found. Run `pnpm --filter @hiveory/nectar-mcp build`. Looked in: {:?}",
        looked
    ))
}
...
/// PATH for spawned WorkerBees — process PATH plus common npm/Node bin dirs.
...
/// Locate a WorkerBee CLI on disk (PATH + npm global bins + known package bins).
...
/// workhive so every agent working in that folder can see it.
#[tauri::command]
...
async fn nectar_ensure_structure(
    ...
    // get_nectar_mcp_path and other repo-relative resolution; the frontend
...
// HoneyBoard's Open-VSX component embeds a local `openvscode-server` (which uses
...
async fn start_openvsx(
    ...
    // so anything set here (HIVEORY_PANE_ID, HIVEORY_QUEEN) reaches them — the
    // same way a WorkerBee's pty passes it to a CLI agent.
