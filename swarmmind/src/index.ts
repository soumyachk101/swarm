 ...
 *
 * Browser/renderer hosts must import `@swarm/mind/core` and supply their
...
export interface SwarmMindConfig {
  projectPath: string;
  pheromoneProjectPath?: string;
...
export class SwarmMind {
  ...
  constructor(config: SwarmMindConfig) {
    ...
    this.handoffs = new HandoffManager(
      config.pheromoneProjectPath || config.projectPath,
  ...
  static async create(config: SwarmMindConfig): Promise<SwarmMind> {
    const hm = new SwarmMind(config);
    await hm.initialize();
    return hm;
  }
