  ...
  return (
    <div
      ...
      className={`
        ...
        ${isDragging ? 'bg-swarm-gold' : 'bg-swarm-border/60 hover:bg-swarm-gold/60'}
        transition-colors duration-150 flex items-center justify-center
        ${className}
      `}
      onMouseDown={handleMouseDown}
    >
      <GripVertical size={12} className="text-swarm-textMuted opacity-0 hover:opacity-100 transition-opacity" />
    </div>
  );
}
