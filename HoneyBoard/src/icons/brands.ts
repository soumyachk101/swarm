...
/**
 * CLI agent id -> its own logo. Keyed by plain string on purpose: HoneyBoard is
 * the shared UI kit and must not depend on WorkerBees (WorkerBees depends on
 * HoneyBoard, so the arrow only points one way).
 */
