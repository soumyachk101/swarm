/**
 * Hiveory's own marks. Every concept the product invented gets a drawn mark,
 * never a stock glyph: a WorkerBee is not a robot, a workhive is not a plain
 * hexagon, Nectar is not a database cylinder, TaskComb is not a kanban board.
 *
...
/** A worker bee: striped abdomen, two wings, antennae. */
export function WorkerBeeMark({ size = 16, className, filled }: MarkProps) {
  return (
    <svg
      {...base(size, className)}
      fill="none"
...
/** A workhive: one outer cell holding three brood cells. */
export function WorkHiveMark({ size = 16, className, filled }: MarkProps) {
  return (
    <svg
      {...base(size, className)}
      fill="none"
...
/** Nectar: a honey droplet holding a stored spark. Memory, not a database. */
export function NectarMark({ size = 16, className, filled }: MarkProps) {
  return (
    <svg
      {...base(size, className)}
      fill="none"
...
/** TaskComb: four comb cells, the board's own mark. */
export function TaskCombMark({ size = 16, className, filled }: MarkProps) {
  return (
    <svg
      {...base(size, className)}
      fill={filled ? "currentColor" : "none"}
