...
describe('buildBreakdownPrompt', () => {
  it('includes the goal and pheromone context in the prompt', () => {
    const prompt = buildBreakdownPrompt({ goal: 'Fix all bugs', pheromoneContext: 'Project uses TypeScript' });
    expect(prompt).toContain('Fix all bugs');
    expect(prompt).toContain('Project uses TypeScript');
  });

  it('handles missing pheromone context', () => {
